...
<div class="step-guide">
  ...
  <div class="step-item">
    ...
    <p>配布されているZIPファイルを、そのまま読み込めます</p>
    <ol>
      <li>プラグイン画面の「プラグインを追加」を開く</li>
      <li>「ZIP から追加」を押してZIPを選ぶ（画面にZIPを<strong>ドラッグ&amp;ドロップ</strong>してもかまいません）</li>
      <li>内容を確かめて「追加する」を押す</li>
      <li>一覧に出てくるので、選んで「追加する」を押すと使えるようになります</li>
    </ol>
    <div class="tips-box">
      <h4>読み込んだだけでは動きません</h4>
      <p>ZIPを読み込むと一覧に名前が並びますが、その時点ではまだ動いていません。用途と前提条件を見てから「追加する」を押してください。<strong>ゆかコネNEOの再起動は要りません。</strong></p>
    </div>
    <div class="tips-box">
      <h4>うまくいかないとき</h4>
      <p>ZIPの中身がプラグインとして見つからないときは、何が入っていたかが表示されます。プラグインは<strong>DLLファイルが入ったフォルダ</strong>が必要です。フォルダが二重になっていたり、説明書や画像が一緒に入っていたりしてもかまいません。</p>
      <p>ゆかコネNEOをProgram Filesなど書き込めない場所に入れている場合は、コピー先のフォルダが開くので、そこへ手でコピーしてください。</p>
    </div>
    <p>以前からの方法（一度終了して、プラグインのフォルダを手でコピーし、メンテナンスツールでロード設定をする）も引き続き使えます。</p>
  </div>
</div>
