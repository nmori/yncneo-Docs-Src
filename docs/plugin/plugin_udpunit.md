...
|設定|意味|既定|
|:--|:---|:--|
|送付先|その通信先に送るかをIPアドレス(v4)で指定します|127.0.0.1|
|送信ポート|送付先通信ポート番号を指定します|39538|
|送信内容|どの内容を送るかを指定します|母国語|

!!! Tips "データについて"
    * データはUTF8で、**音声認識が確定したときだけ**送られます。認識の途中経過は送られません。

!!! Info "このプラグインはAPIから操作できません"
    ``/api/command`` には対応していないため、外部から送信を発火させることはできません。
