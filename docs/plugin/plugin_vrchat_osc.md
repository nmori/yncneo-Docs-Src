...
|設定|意味|
|:--|:---|
|VRChat音声ミュート連動(linking Mute)|VRChat側のマイクのミュートに合わせて、字幕のミュートを切り替えます|
|連動のしかた|`そのまま` はVRChatでミュートしたら字幕もミュートします。`反転` は逆で、VRChatでミュートしている間だけ字幕を出します|
|ミュートしたらチャットの字幕も消す(Clear chatbox)|字幕を出したままミュートしたときに、VRChatのチャットボックスに残った字幕を消します|
|左ジェスチャー連動(linking Left Gesture)|左手のジェスチャーに合わせて合成音声の声色を変えます|

!!! Info "連動のしかた"
    * `VRChat音声ミュート連動` がONのときだけ選べます。
    * `反転` は、VRChatではしゃべらずに字幕だけで会話したいときに使います。

!!! Info "ミュートしたらチャットの字幕も消す"
